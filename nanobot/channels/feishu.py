...
class FeishuChannel(BaseChannel):
    ...
    def _upload_audio_from_path(self, path: str) -> str | None:
        """
        Upload a local audio to Feishu and return audio_key.
        """
        if not self._client:
            return None
    ...
    async def send(self, msg: OutboundMessage) -> None:
        ...
        try:
            ...
            #    - Audio extensions -> file upload, send as audio
            #    - Others -> file upload
            image_exts = {".png", ".jpg", ".jpeg", ".gif", ".bmp", ".webp"}
            audio_exts = {".mp3", ".m4a", ".wav", ".ogg", ".amr", ".aac", ".opus"}
            image_keys: list[str] = []
            audio_keys: list[str] = []
            file_keys: list[str] = []
            for item in msg.media or []:
                ...
                if isinstance(item, str):
                    # Simple heuristic: http/https URL -> treat as remote image/audio/file
                    if item.startswith("http://") or item.startswith("https://"):
                        ...
                        if ext in image_exts:
                            ...
                            key = self._upload_audio_from_url(item)
                            if key:
                    ...
                    if os.path.isfile(local_candidate):
                        ...
                        continue
                    # Raw Feishu keys: audio:file_xxx -> audio_key, file_xxx -> file_key, else image_key
                    if item.startswith("audio_"):
                        logger.debug(f"Treating media item as Feishu audio file_key: {item}")
                        audio_keys.append(item)
                    elif item.startswith("file_"):
            ...
            # 5) Send audio if present (file_key, msg_type "audio" for inline playback)
            for audio_key in audio_keys:
                ...
                try:
                    ...
                    audio_response = self._client.im.v1.message.create(audio_request)
                    if not audio_response.success():
                        logger.error(
                            f"Failed to send Feishu audio: code={audio_response.code}, "
                            f"msg={audio_response.msg}, log_id={audio_response.get_log_id()}"
                        )
                    else:
                        logger.debug(f"Feishu audio sent to {msg.chat_id}")
                except Exception as e:
